...
import { MapPin, Navigation, Clock, CheckCircle2, ChevronRight, Package, Store } from 'lucide-react';

export default function RiderDashboard() {
  const [status, setStatus] = useState('online');

  const mockOrders = [
    { id: 'OLY-491204', restaurant: 'Spicy Dragon', distance: '1.2 km', dropoff: '4.5 km', earn: '₹45', time: '5 mins ago' },
    { id: 'OLY-782391', restaurant: 'Green Leaf', distance: '0.8 km', dropoff: '2.1 km', earn: '₹30', time: '12 mins ago' },
  ];
      ...
      <header className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Lat: 19.0760 • Lng: 72.8777</p>
          </div>
        </div>
      ...
      <main className="p-6 max-w-lg mx-auto">
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-gradient-to-br from-slate-800 to-slate-800/50 p-5 rounded-2xl border border-slate-700/50 shadow-xl relative overflow-hidden">
            ...
            <h2 className="text-3xl font-bold text-white">₹845</h2>
          </div>
          <div className="bg-gradient-to-br from-slate-800 to-slate-800/50 p-5 rounded-2xl border border-slate-700/50 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center">
              <span className="w-2.5 h-2.5 rounded-full bg-indigo-400 animate-ping absolute"></span>
...
function OrderCard({ id, restaurant, distance, dropoff, earn, time }) {
  return (
    <div className="bg-slate-800/60 backdrop-blur-md rounded-2xl p-5 border border-slate-700/50 shadow-2xl hover:border-orange-500/30 transition-all duration-300 group cursor-pointer">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <span className="text-xs font-medium text-slate-400 bg-slate-900/50 px-2 py-1 rounded-md border border-slate-700">{id}</span>
        </div>
        <div className="text-right">
          <p className="text-2xl font-black text-emerald-400">{earn}</p>
          <p className="text-xs text-slate-400 flex items-center justify-end gap-1 mt-1"><Clock className="w-3 h-3" /> {time}</p>
        </div>
      </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{dropoff} ride</p>
            </div>
          </div>
        </div>
      </div>

      <button className="w-full py-3.5 bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-400 hover:to-rose-400 text-white font-bold rounded-xl shadow-lg shadow-orange-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 group-hover:shadow-orange-500/40">
