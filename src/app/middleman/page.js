...
function MiddlemanSidebar({ activeTab, setActiveTab, isMobileOpen, setIsMobileOpen }) {
  const menuItems = [
    { id: 'dashboard', icon: <BarChart3 className="w-5 h-5" />, label: 'Middleman Dashboard', count: null },
    { id: 'restaurants', icon: <ShoppingBag className="w-5 h-5" />, label: 'Restaurants', count: '24' },
    { id: 'delivery', icon: <Truck className="w-5 h-5" />, label: 'Delivery Partners', count: '42' },
    { id: 'orders', icon: <Package className="w-5 h-5" />, label: 'Active Orders', count: '18' },
    { id: 'disputes', icon: <AlertTriangle className="w-5 h-5" />, label: 'Disputes', count: '3' },
    { id: 'commission', icon: <Percent className="w-5 h-5" />, label: 'Commission', count: null },
  ];
  ...
  return (
    <>
      ...
      <button
        ...
        className="xl:hidden fixed top-4 left-4 z-50 p-2 bg-gray-800 border border-gray-700 rounded-lg shadow-lg"
      >
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-gray-900 to-gray-950 border-r border-gray-800
      ...
      `}>
          ...
          <div className="px-4">
            <div className="space-y-1">
              {menuItems.map((item) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-5 h-5 flex items-center justify-center ${activeTab === item.id ? 'text-green-400' : 'text-gray-400'}`}>
                      {item.icon}
                    </div>
                    <span className="text-sm font-medium">{item.label}</span>
                  </div>
                  {item.count && (
                    <span className={`px-2 py-1 text-xs rounded ${activeTab === item.id ? 'bg-green-500 text-white' : 'bg-gray-800 text-gray-400'}`}>
                      {item.count}
                    </span>
                  )}
                </button>
        ...
        <button
          ...
          className="xl:hidden absolute top-4 -right-0 p-2"
        >
          <X className="w-5 h-5" />
        </button>
      </aside>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className={`p-3 rounded-lg bg-gradient-to-br ${stat.color} bg-opacity-10 group-hover:scale-110 transition-transform`}>
                <Icon className="w-6 h-6" style={{ color: stat.color.split(' ')[0].split('-')[1] }} />
              </div>
...
function CoordinationTimeline() {
  const events = [
    { time: '9:00 AM', type: 'order', restaurant: 'Pizza Palace', delivery: 'John D', status: 'assigned', icon: <Package className="w-4 h-4" /> },
    { time: '10:30 AM', type: 'pickup', restaurant: 'Burger King', delivery: 'Mike R', status: 'completed', icon: <CheckCircle className="w-4 h-4" /> },
    { time: '12:15 PM', type: 'dispute', restaurant: 'Sushi Bar', delivery: 'Sarah E', status: 'resolved', icon: <AlertTriangle className="w-4 h-4" /> },
    { time: '2:45 PM', type: 'order', restaurant: 'Coffee Shop', delivery: 'Alex S', status: 'in_progress', icon: <Package className="w-4 h-4" /> },
    { time: '4:20 PM', type: 'commission', restaurant: 'Pasta House', delivery: 'David L', status: 'paid', icon: <IndianRupee className="w-4 h-4" /> },
    { time: '6:30 PM', type: 'rating', restaurant: 'Pizza Palace', delivery: 'John D', status: 'received', icon: <Star className="w-4 h-4" /> },
  ];
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-bold text-white">Today's Coordination Timeline</h3>
          <p className="text-gray-400 text-sm">Real-time coordination activities</p>
        </div>
      ...
      <div className="space-y-4">
        {events.map((event, index) => (
          <div key={index} className="flex items-center p-4 rounded-lg bg-gray-800/30 hover:bg-gray-800/50 transition-colors">
            {/* Timeline line */}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-medium text-white">
                    {event.type.charAt(0).toUpperCase() + event.type.slice(1)} • {event.restaurant}
                  </h4>
                  <p className="text-xs text-gray-400 mt-1">Delivery: {event.delivery}</p>
                </div>
                <div className="flex items-center space-x-3">
                  <span className={`px-3 py-1 text-xs rounded-full ${statusColors[event.status]}`}>
...
function ActiveOrders() {
  ...
  const handleAssign = (orderId, deliveryPartner) => {
    setOrders(orders.map(order => 
      order.id === orderId ? { ...order, status: 'assigned', delivery: deliveryPartner } : order
    ));
  };
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-bold text-white">Active Orders Coordination</h3>
          <p className="text-gray-400 text-sm">Manage and assign orders to delivery partners</p>
        </div>
      ...
      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="p-4 rounded-lg bg-gray-800/30 hover:bg-gray-800/50 transition-colors">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              {/* Order Info */}
              <div className="flex-1">
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    order.status === 'coordinating' ? 'bg-yellow-500/20' :
                    order.status === 'assigned' ? 'bg-blue-500/20' :
                    order.status === 'picked_up' ? 'bg-purple-500/20' :
                    'bg-green-500/20'
                  }`}>
                    <Package className={`w-5 h-5 ${
                      order.status === 'coordinating' ? 'text-yellow-400' :
                      order.status === 'assigned' ? 'text-blue-400' :
                      order.status === 'picked_up' ? 'text-purple-400' :
                      'text-green-400'
                    }`} />
                  </div>
                  <div>
                    <div className="flex items-center space-x-2">
                      <h4 className="text-sm font-medium text-white">{order.id}</h4>
                      <span className="text-xs text-gray-400">•</span>
                      <span className="text-sm text-gray-300">{order.restaurant}</span>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex flex-col items-start">
                  <div className="text-right">
                    <div className="text-sm font-bold text-white">{order.commission}</div>
                ...
                <div className="flex items-center space-x-2">
                  <span className={`px-3 py-1.5 text-xs rounded-full ${statusColors[order.status]}`}>
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Manage restaurant partnerships and coordination</p>
        </div>
        <button className="text-sm text-green-400 hover:text-green-300 flex items-center">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-800">
            {restaurants.map((restaurant) => (
              <tr key={restaurant.id} className="hover:bg-gray-800/30 transition-colors">
                ...
                <td className="py-4 px-4">
                  <div className="text-sm font-bold text-white">{restaurant.orders}</div>
                ...
                <td className="py-4 px-4">
                  <div>
                    <div className="text-sm text-white">{restaurant.contact}</div>
                    <div className="text-xs text-gray-500">{restaurant.phone}</div>
                  </div>
                </td>
                <td className="py-4 px-4">
                  <span className={`px-3 py-1 text-xs rounded-full ${statusColors[restaurant.status]}`}>
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Manage and coordinate with delivery partners</p>
        </div>
        <div className="flex items-center space-x-2">
          <button className="text-sm text-green-400 hover:text-green-300 flex items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
        {partners.map((partner) => (
          <div key={partner.id} className="p-4 rounded-lg bg-gray-800/30 hover:bg-gray-800/50 transition-colors border border-gray-800">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`px-2 py-1 text-xs rounded-full ${statusColors[partner.status]}`}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="text-center p-2 bg-gray-800/50 rounded-lg">
                ...
                <div className="text-sm font-bold text-white">{partner.orders}</div>
              </div>
              <div className="text-center p-2 bg-gray-800/50 rounded-lg">
                <div className="text-xs text-gray-400">Rating</div>
                <div className="text-sm font-bold text-yellow-400">{partner.rating}</div>
              </div>
            </div>
            ...
            <div className="mt-4 text-xs text-gray-400">
              <div className="flex items-center justify-between">
                ...
                <div className="text-green-400 font-medium">{partner.earnings}</div>
              </div>
            </div>
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">Resolve conflicts between restaurants and delivery</p>
        </div>
        {/* <div className="flex items-center space-x-2">
          <button className="p-2 hover:bg-gray-800 rounded-lg">
            <Filter className="w-4 h-4 text-gray-400" />
      ...
      <div className="space-y-4">
        {disputes.map((dispute) => (
          <div key={dispute.id} className="p-4 rounded-lg bg-gray-800/30 hover:bg-gray-800/50 transition-colors">
            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
              {/* Left Column */}
              <div className="flex-1">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h4 className="text-sm font-medium text-white">{dispute.type}</h4>
                    <div className="text-xs text-gray-400 mt-1">
                      {dispute.restaurant} • {dispute.delivery} • Customer: {dispute.customer}
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className={`px-2 py-1 text-xs rounded-full ${priorityColors[dispute.priority]}`}>
                      {dispute.priority}
                    </span>
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Monthly commission earnings and trends</p>
        </div>
        {/* <div className="flex items-center space-x-2">
          <button className="text-sm text-green-400 hover:text-green-300 flex items-center">
        ...
        {commissions.map((commission, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-center flex-1">
              ...
              <div className="mt-2 text-xs text-gray-500">{commission.month}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-gray-800">
        <div className="text-center p-3 bg-gray-800/30 rounded-lg">
          <div className="text-lg font-bold text-green-400">₹{commissions[commissions.length - 1].amount}</div>
        ...
        <div className="text-center p-3 bg-gray-800/30 rounded-lg">
          <div className="text-lg font-bold text-blue-400">{commissions[commissions.length - 1].orders}</div>
          <div className="text-xs text-gray-500">Orders This Month</div>
        </div>
        <div className="text-center p-3 bg-gray-800/30 rounded-lg">
          <div className="text-lg font-bold text-yellow-400">
            ₹{(commissions.reduce((sum, c) => sum + c.amount, 0) / commissions.reduce((sum, c) => sum + c.orders, 0)).toFixed(2)}
          </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-800">
        <h4 className="text-sm font-medium text-white mb-4">Recent Commission Transactions</h4>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">Pizza Palace • Today, 2:30 PM</div>
              </div>
            </div>
            <div className="text-right">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">Burger King • Today, 1:15 PM</div>
              </div>
            </div>
            <div className="text-right">
...
export default function MiddlemanDashboard() {
          ...
          <>
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-white">Restaurant Partners</h2>
              ...
              <p className="text-gray-400">Manage restaurant partnerships and coordination</p>
            </div>
        ...
        return (
          <>
            <div className="mb-6">
              ...
              <p className="text-gray-400">Manage and coordinate with delivery partners</p>
            </div>
        ...
        return (
          <>
            <div className="mb-6">
              ...
              <p className="text-gray-400">Coordinate and manage active orders</p>
            </div>
        ...
        return (
          <>
            <div className="mb-6">
              ...
              <p className="text-gray-400">Resolve conflicts between restaurants and delivery partners</p>
            </div>
        ...
        return (
          <>
            <div className="mb-6">
              ...
              <p className="text-gray-400">Track commission earnings and performance</p>
            </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-950 text-gray-300">
      ...
      <div className="flex-1 ml-0 xl:ml-64">
        {/* Header */}
        <header className="sticky top-0 z-30 bg-gray-900 border-b border-gray-800 p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-white capitalize">
                {activeTab === 'dashboard' ? 'Middleman Dashboard' : 
                 activeTab === 'restaurants' ? 'Restaurant Partners' :
                 activeTab === 'delivery' ? 'Delivery Partners' :
                 activeTab === 'orders' ? 'Active Orders' :
                 activeTab === 'disputes' ? 'Dispute Resolution' :
                 activeTab === 'commission' ? 'Commission Management' : activeTab}
              </h2>
              <p className="text-gray-400">
                {activeTab === 'dashboard' ? 'Welcome to your coordination panel' : 
                 activeTab === 'restaurants' ? 'Manage restaurant partnerships and coordination' :
                 activeTab === 'delivery' ? 'Manage and coordinate with delivery partners' :
                 activeTab === 'orders' ? 'Coordinate and manage active orders' :
                 activeTab === 'disputes' ? 'Resolve conflicts between restaurants and delivery partners' :
                 activeTab === 'commission' ? 'Track commission earnings and performance' :
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder={
                    activeTab === 'restaurants' ? 'Search restaurants...' :
                    activeTab === 'delivery' ? 'Search delivery partners...' :
                    activeTab === 'orders' ? 'Search orders...' :
                    activeTab === 'disputes' ? 'Search disputes...' :
                    activeTab === 'commission' ? 'Search transactions...' :
                    'Search...'
                  }
                  value={searchQuery}
      ...
      <button className="fixed bottom-6 right-6 w-12 h-12 bg-gradient-to-r from-green-500 to-emerald-500 
